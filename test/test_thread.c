...
struct asyncMsg g_asyncMsg[2] = {
    {MODULE_TEST1_EVENT2, test1event1AsyncFunc},
...
{
    char recv[200];
    char send[200] = "thread data";
    // 发个消息自己
    PostNotify(MODULE_TEST2, MODULE_TEST1_EVENT1, recv, sizeof(recv), send, sizeof(send));
    while (1) {
        sleep(10);
        printf("10 second send %s to module1 event1\n", send);
        SendNotify(MODULE_TEST1, MODULE_TEST1_EVENT1, send, sizeof(send), recv, sizeof(recv));
        PostNotify(MODULE_TEST1, MODULE_TEST1_EVENT2, recv, sizeof(recv), send, sizeof(send));
        printf("read data from module1, %s\n\n", recv);
        sleep(10);
        printf("10 second send %s to module1 event2\n", send);
        SendNotify(MODULE_TEST1, MODULE_TEST1_EVENT2, send, sizeof(send), recv, sizeof(recv));
        PostNotify(MODULE_TEST1, MODULE_TEST1_EVENT1, recv, sizeof(recv), send, sizeof(send));
        printf("read data from module1, %s\n\n", recv);
    }
}
...
int main(void)
{
    ...
    if (pthread_create(&tid, NULL, (void *)printEvent, NULL) != 0) {
    ...
    while(1) {
        ...
        printf("get user name from module2 %s\n", recv);
